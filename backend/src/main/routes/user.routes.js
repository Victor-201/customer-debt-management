...
import { createUserSchema, updateUserSchema } from "../../presentation/validators/user.schema.js";
...
router.post("/", permissionMiddleware(USER_PERMISSIONS.CREATE), validateMiddleware(createUserSchema), controller.createUser);
router.get("/", permissionMiddleware(USER_PERMISSIONS.READ), controller.getAllUsers);
router.get("/deleted", permissionMiddleware(USER_PERMISSIONS.READ), controller.getDeletedUsers);

router.get("/:userId", permissionMiddleware(USER_PERMISSIONS.READ), controller.getUser);
router.patch("/:userId", permissionMiddleware(USER_PERMISSIONS.UPDATE), validateMiddleware(updateUserSchema), controller.updateUser);

router.post("/:userId/lock", permissionMiddleware(USER_PERMISSIONS.LOCK), controller.lockUser);
router.post("/:userId/unlock", permissionMiddleware(USER_PERMISSIONS.LOCK), controller.unlockUser);

router.delete("/:userId/soft", permissionMiddleware(USER_PERMISSIONS.DELETE), controller.softDeleteUser);
router.post("/:userId/restore", permissionMiddleware(USER_PERMISSIONS.DELETE), controller.restoreUser);

router.delete("/:userId/hard", permissionMiddleware(USER_PERMISSIONS.DELETE), controller.hardDeleteUser);
router.delete("/hard/all", permissionMiddleware(USER_PERMISSIONS.DELETE), controller.hardDeleteAllDeletedUsers);
